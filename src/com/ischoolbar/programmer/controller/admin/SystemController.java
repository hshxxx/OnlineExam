...
public class SystemController {
	...
	public ModelAndView login(ModelAndView model){
		model.setViewName("system/login"); //这个路径是web-Inf下views的路径
		return model;
	}
	...
	public Map<String, String> loginAct(User user,String cpacha,HttpServletRequest request){
		...
		//说明用户名密码及验证码都正确
		//此时需要查询用户的角色权限
		Role role = roleService.find(findByUsername.getRoleId()); //根据角色ID找到用户
		List<Authority> authorityList = authorityService.findListByRoleId(role.getId());//根据角色获取权限列表
		String menuIds = "";
		for(Authority authority:authorityList){
			menuIds += authority.getMenuId() + ",";
		}
		if(!StringUtils.isEmpty(menuIds)){ //去掉最后一个逗号 
